...
import config
from unir import (banda_cvss, calcular_riesgo_base, calcular_riesgo_operativo,
                  clasificar_prioridad, resumen)
...
def main():
    ...
    print("\n== Puntaje de riesgo ==")
    riesgo_critica = calcular_riesgo_base(fila(cvss=9.8, epss=0.001))
    riesgo_media = calcular_riesgo_base(fila(cvss=6.5, epss=0.35))
    ok(riesgo_media > riesgo_critica,
       f"una media probable ({riesgo_media}) supera a una crítica improbable ({riesgo_critica})",
       fallos)
    ok(calcular_riesgo_base(fila(cvss=None, epss=0.5)) == 0.0,
       "sin CVSS el riesgo queda en cero, no en error", fallos)
    ok(calcular_riesgo_base(fila(cvss=10.0, epss=1.0)) == 10.0,
       "el máximo teórico es 10", fallos)

    print("\n== Separación entre riesgo de evaluación y riesgo operativo ==")
    # Esta distinción evita un error de método: si el puntaje usado para medir
    # el acierto ya conoce el catálogo de explotación, mide su propia respuesta.
    explotada = fila(cvss=5.3, epss=0.02, explotada=True)
    explotada["riesgo_base"] = calcular_riesgo_base(explotada)
    ok(calcular_riesgo_operativo(explotada) == 10.0,
       "el riesgo operativo pone al máximo lo que ya se explota", fallos)
    ok(calcular_riesgo_base(explotada) < 1.0,
       "el riesgo base ignora el catálogo y deja esa misma vulnerabilidad baja", fallos)

    normal = fila(cvss=8.0, epss=0.25, explotada=False)
    normal["riesgo_base"] = calcular_riesgo_base(normal)
    ok(calcular_riesgo_operativo(normal) == normal["riesgo_base"],
       "sin explotación confirmada ambos puntajes coinciden", fallos)
