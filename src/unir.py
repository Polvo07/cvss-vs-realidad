...
def calcular_riesgo_base(fila):
    """
    Riesgo estimado SIN mirar el catálogo de explotación confirmada.
    ...
    Existe separado del riesgo operativo porque es el único que sirve para
    evaluar el método. Si el puntaje usara el catálogo KEV, al ordenar por él
    las explotadas quedarían de primeras por construcción, y medir cuántas
    "acierta" sería usar la respuesta para predecir la respuesta.
    """
...
def calcular_riesgo_operativo(fila):
    """
    Riesgo para usar en el día a día, este sí con el catálogo incluido.

    Aquí usar KEV es correcto y deseable: si una vulnerabilidad ya se está
    explotando, parcharla primero no se discute. La distinción con el riesgo
    base es de propósito, no de calidad: uno sirve para decidir, el otro para
    evaluar si el método de decisión funciona.
    """
    if fila["explotada"]:
        return 10.0
    return fila["riesgo_base"]
...
def unir():
    ...
    tabla["riesgo_base"] = tabla.apply(calcular_riesgo_base, axis=1)
    tabla["riesgo"] = tabla.apply(calcular_riesgo_operativo, axis=1)
